...
export function renderMessageByFormats(businesses) {
  return {
    ...
    discord: renderDiscordMessage(businesses),
  };
}
...
function renderDiscordMessage(businesses) {
  return businesses
    .map(
      (business) =>
        `🍽 [${business.display_name}](<https://share.toogoodtogo.com/item/${business.item.item_id
        }>)
💰 ${business.item.price_including_taxes.minor_units / 100}€
🥡 ${business.items_available}
⏰ ${formatInterval(business)}`
